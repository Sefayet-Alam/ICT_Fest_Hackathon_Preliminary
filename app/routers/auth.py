...
# Serializes the check-then-insert of org + user so concurrent registrations
# resolve deterministically (first creator of a new org is admin, the rest join
# as members) instead of racing the unique constraints into a 500.
...
def refresh(payload: RefreshRequest, db: Session = Depends(get_db)):
    ...
    if is_revoked(data["jti"]):
        raise AppError(401, "UNAUTHORIZED", "Refresh token already used")
    user = db.query(User).filter(User.id == int(data["sub"])).first()
    if user is None:
        raise AppError(401, "UNAUTHORIZED", "Unknown user")
    # Single-use: consume the presented refresh token so it cannot be reused.
    revoke_jti(data["jti"])
    return {
        "access_token": create_access_token(user),
        "refresh_token": create_refresh_token(user),
