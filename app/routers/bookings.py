...
# Serializes the check-then-write critical sections (conflict, quota, reference
# code, cancel) so they hold under concurrent requests on single-writer SQLite.
...
def _has_conflict(db: Session, room_id: int, start: datetime, end: datetime) -> bool:
    ...
    for b in existing:
        ...
        # Strict comparisons allow back-to-back bookings (one ending exactly
...
def create_booking(
    ...
    # Conflict + quota check and the insert must be atomic so no two concurrent
    # requests can both pass the checks and double-book / exceed the quota.
    with _booking_lock:
...
def cancel_booking(
    ...
    # Serialize concurrent cancels of the same booking so exactly one succeeds
    # and exactly one RefundLog is written.
    with _booking_lock:
        db.refresh(booking)
        ...
        # Single source of truth: the RefundLog entry's amount is what we return,
        # so the response always equals the stored refund.
